...
| 读到的内容 | 单站按日：全天合计 / 各班次金额·油量·时段·交班状态 / 全天按油品（升数与笔数）/ 逐笔明细，可导出 CSV |
...
│  ├─ HomeView.swift               # 首页：站点目录（只列站名，不展示金额、不做批量查询）
│  ├─ StationDetailView.swift      # 单站详情：选营业日期 → 手动查询 → 全天合计 + 各班次 + 按油品（升数）+ 明细
│  ├─ StyleKit.swift               # 统一排版组件（信息行 / 状态胶囊 / 指标块 / 空状态）
...
- 首页就是站点目录：**只列站名，不显示任何金额，也没有批量查询**（首页不发任何云库请求，不做并发连库，避免真机闪退）。点站名进详情。
- 详情页自带**查询方式切换**：默认「单日」，打开开关切到「营业日区间」。
  - **单日**：可点日历选任意一天，另有「今天 / 昨天 / 前天」快捷按钮；选好日期后点「查询当日数据」才会连库。改日期不会自动重查，会提示重新查询。
  - **营业日区间**：可选任意起止营业日（如 `2026-08-01` 至 `2026-09-01`），另有「近7天 / 近30天 / 本月」快捷区间；点「查询区间营业额」后给出区间合计（营业额 / 油量 / 交易笔数 / 首末笔时间 / 有数据天数）、**按营业日拆分**、**按油品拆分**（沿用「只显升数、不显支付方式」口径），并可将汇总导出为 CSV。
- 详情页顺序（单日）：**全天合计（营业额 / 油量 / 笔数）→ 各班次明细（班次 / 营业额 / 油量·笔数 / 时段 / 状态）→ 全天按油品（只显示升数与笔数）→ 逐笔明细**；区间模式则依次为：**区间合计 → 按营业日 → 区间按油品 → 逐笔明细**。右上角第一个按钮 = 复制全部（粘贴到微信/备忘录即为电脑端同款文本）。
- 逐笔明细**不设笔数上限**：单日查询一次加载当日全部明细；区间查询按每页 200 笔分页，界面一次最多渲染 200 行（避免长区间滑不动），点「继续加载下一页」追加，导出可拿到已加载的全部明细。
- 状态配色与电脑端一致：**已交班 = 绿字**（#0b7a3b），**未交班(营业中) = 红字**（#b3261e），未到交班点 = 红字提示。
- 逐笔明细可导出 CSV，可发微信/邮件，Excel 直接打开不乱码。
...
2. 逐笔明细已**取消 500 笔上限**：单日查询一次拉取当日全部明细；营业日区间查询按每页 200 笔分页加载，界面一次最多渲染 200 行（其余靠「继续加载下一页」与 CSV 导出获取）。
