...
- 详情页自带**营业日期选择**（可点日历选任意一天，另有「今天 / 昨天 / 前天」快捷按钮）；选好日期后点「查询当日数据」才会连库。改日期不会自动重查，会提示重新查询。
- 详情页顺序：**全天合计（营业额 / 油量 / 笔数）→ 各班次明细（班次 / 营业额 / 油量·笔数 / 时段 / 状态）→ 全天按油品（只显示升数与笔数）→ 逐笔明细**；右上角第一个按钮 = 复制全部（粘贴到微信/备忘录即为电脑端同款文本）。
